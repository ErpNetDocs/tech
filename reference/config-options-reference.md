...
- a.Action: When the value of this key is:
    - "1" - When Scan single barcode then this is accepted as 1 PCE
...
- a.Action: If thе key is defined and has a value, it determines the retention period, in months, for the records in the [Attribute Changes](xref:Systems.Core.AttributeChanges) table, which are generated by the [Track Changes](~/advanced/data-objects/track-changes.md) system. Otherwise, the retention period is set to 36 months by default.<br>
If the [J35666 Delete old history data for attribute changes](~/advanced/jobs/J35666.md) job is configured and active - the old history data for attribute changes, whose retention period is expired, will be deleted.
- c.The user sets it manually.
