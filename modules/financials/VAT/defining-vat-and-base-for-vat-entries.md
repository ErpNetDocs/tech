...
The VAT additional amount is set in the @@name definition (see <b>Additional Amounts</b>) of the current document. If the document does not contain the specified VAT additional amount (or there is no such in the @@name company definition), the calculation is interrupted by an error message. 
