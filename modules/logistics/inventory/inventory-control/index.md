...
You can include format specifiers and combine multiple fields as needed. You’ll see the configured format applied when browsing orders before starting operations like [Receive](receive.md) or [Issue](issue.md). For more details, see the **[Config Options Reference](xref:config-options-reference.md)** (option 70).
